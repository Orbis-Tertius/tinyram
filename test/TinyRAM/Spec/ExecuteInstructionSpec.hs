...
import           TinyRAM.Types.Instruction         (BinOp (..), Comp (..),
                                                    Instruction (..), UnOp (..))
...
instructionStateTransition ps i =
  case i of
    And p   -> functionOpcode (.&.) (\x y -> conditionToFlag (x .&. y == 0)) p ws

    Or p    -> functionOpcode (.|.) (\x y -> conditionToFlag (x .|. y == 0)) p ws

    Xor p   -> functionOpcode xor   (\x y -> conditionToFlag (x `xor` y == 0)) p ws

    Not p   -> functionOpcode1 (\a -> (2 ^ ws - 1) `xor` a) (\a -> conditionToFlag (complement a == 0)) p ws

    Add p   -> functionOpcode (\x y -> (x + y) .&. wordSizeBitmask)
                         (\x y -> conditionToFlag $ (x + y) .&. wordSizeBitmaskMSB /= 0)
                         p ws

    Sub p   -> functionOpcode (\x y -> (y + wordStrictBound - x) .&. wordSizeBitmask)
                         (\x y -> conditionToFlag ((y + wordStrictBound - x) .&. wordSizeBitmaskMSB == 0))
                         p ws

    Mull p  -> functionOpcode (\x y -> (x * y) .&. wordSizeBitmask)
                         (\x y -> conditionToFlag ((x * y) .&. wordSizeBitmaskMSB /= 0))
                         p ws

    Umulh p -> functionOpcode (\x y -> (x * y) `shift` negate (ws ^. #unWordSize))
                         (\x y -> conditionToFlag ((x * y) .&. wordSizeBitmaskMSB /= 0))
                         p ws

    Smulh p -> functionOpcode (\x y -> unSignedInt $ signedMultiplyHigh ws (SignedInt x) (SignedInt y))
    ...
    Udiv p  -> functionOpcode (\x y -> if x == 0 then 0 else (y `div` x) .&. wordSizeBitmask)
                         (\x _ -> conditionToFlag (x == 0))
                         p ws

    Umod p -> functionOpcode (\x y -> if x == 0 then 0 else (y `mod` x) .&. wordSizeBitmask)
                         (\x _ -> conditionToFlag (x == 0))
                         p ws

    Shl p -> functionOpcode (\x y -> (y `shift` fromIntegral (min (fromIntegral ws) x)) .&. wordSizeBitmask)
                         (\_ y -> conditionToFlag $ y .&. (2 ^ (ws ^. #unWordSize - 1)) /= 0)
                         p ws

    Shr p -> functionOpcode (\x y -> (y `shift` negate (fromIntegral (min (fromIntegral ws) x))) .&. wordSizeBitmask)
                         (\_ y -> Flag . fromIntegral $ y .&. 1)
                         p ws

    Cmpe p -> comparisonOpcode (==) p ws

    Cmpa p -> comparisonOpcode (<)  p ws

    Cmpae p -> comparisonOpcode (<=) p ws

    Cmpg p -> comparisonOpcode (\x y -> decodeSignedInt ws (SignedInt x) < decodeSignedInt ws (SignedInt y)) p ws

    Cmpge p -> comparisonOpcode (\x y -> decodeSignedInt ws (SignedInt x) <= decodeSignedInt ws (SignedInt y)) p ws

    Mov (UnOp ri a) -> incrementPC ws
        . (\s -> #registerValues . #unRegisterValues . at ri
              .~ Just (getA a s)
              $ s)

    Cmov (UnOp ri a) -> incrementPC ws
        . (\s ->
...
functionOpcode
  ...
  -> MachineState
  -> MachineState
functionOpcode f p (BinOp ri rj a) ws s =
  incrementPC ws
  .
...
functionOpcode1
  ...
  -> MachineState
  -> MachineState
functionOpcode1 f p (UnOp ri a) ws s =
  incrementPC ws
  .
  (#registerValues . #unRegisterValues . at ri
     .~ Just (f a'))
...
comparisonOpcode
  ...
  -> MachineState
  -> MachineState
comparisonOpcode p (Comp ri a) ws s =
  incrementPC ws
  .
