...
jmpTestExample :: Spec
jmpTestExample = 
  before (handleCommand (CommandParse (AssemblyFilePath "examples/jmpTestExample.s") objectFilePath)) $
    it "The jmp test example should result in 0." $ do
      program <- readObjectFile objectFilePath
      let answer = executeProgram (Params 16 16) (Just 1000) program (InputTape [1,2,3,4]) (InputTape [1,2,3])
      answer `shouldBe` Right (0)
  where 
    objectFilePath = ObjectFilePath "examples/jmpTestExample.o"

jmpTestExampleNonTerm :: Spec
jmpTestExampleNonTerm = 
  before (handleCommand (CommandParse (AssemblyFilePath "examples/jmpTestExampleNonTerm.s") objectFilePath)) $
    it "The jmp test example should result in non termination." $ do
      program <- readObjectFile objectFilePath
      let answer = executeProgram (Params 16 16) (Just 1000) program (InputTape [1,2,3,4]) (InputTape [1,2,3])
      answer `shouldBe` Right (0)
  where 
    objectFilePath = ObjectFilePath "examples/jmpTestExampleNonTerm.o"
...
andTestNegative :: Spec
andTestNegative = 
  before (handleCommand (CommandParse (AssemblyFilePath "examples/andTestNegative.s") objectFilePath)) $
    it "The AND operation should result in 37." $ do
      program <- readObjectFile objectFilePath
      let answer = executeProgram (Params 16 16) (Just 1000) program (InputTape [1,2,3,4]) (InputTape [1,2,3])
      answer `shouldBe` Right 37
  where 
    objectFilePath = ObjectFilePath "examples/andTestNegative.o"
